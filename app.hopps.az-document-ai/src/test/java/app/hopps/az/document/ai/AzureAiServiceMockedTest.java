...
class AzureAiServiceMockedTest
{
	...
	// FIXME: this doesn't work, we currently don't know of a way to create an
	// AnalyzeResult,
	...
	void shouldAnalyzeInvoiceAgainstMock() throws Exception
	{
		...
		// DocumentData documentData = new DocumentData(new URI(url).toURL(),
		// -1L, DocumentType.INVOICE);

		// when

		// scanInvoice now takes a FileUpload as an argument - since this test
		// is currently unused,
		// I've decided not to fix this.

		// Optional<InvoiceData> invoiceData =
		// aiService.scanInvoice(documentData);
