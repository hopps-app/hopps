...
Members are linked to their Keycloak identity via the stable `keycloakId` (the JWT `sub` claim), **not**
the email. Tests that resolve a member must therefore provide the `sub` via `@OidcSecurity`, e.g.:

```java
@TestSecurity(user = "alice@example.test")
@OidcSecurity(claims = @Claim(key = "sub", value = "eb4123a3-b722-4798-9af5-8957f823657a"))
```

For `alice` and `jdoe` the `keycloakId` matches the real users in `quarkus-realm.json`, so dev-mode login
resolves them correctly. All other members use synthetic ids of the form `00000000-0000-0000-0000-0000000000<id>`.

| User | Email | keycloakId (sub) | Organization | Member ID | Common Use Case |
|------|-------|------------------|--------------|-----------|-----------------|
| Alice | alice@example.test | eb4123a3-b722-4798-9af5-8957f823657a | Bühnefrei e.V. | 21 | Primary test user in DocumentResourceTest |
| Jdoe | jdoe@example.test | 1eed6a8e-a853-4597-b4c6-c4c2533546a0 | Bühnefrei e.V. | 22 | Secondary test user |
| Bob | bob@example.test | 00000000-0000-0000-0000-000000000023 | Bühnefrei e.V. | 23 | Test user in TransactionRecordResourceTest |
