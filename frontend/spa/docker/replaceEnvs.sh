...
# Optional: unset means "log in through Keycloak", so these must end up empty rather than keep their placeholder.
STRING_MAP["VITE_OIDC_PROVIDER_URL_VALUE"]=$VITE_OIDC_PROVIDER_URL
STRING_MAP["VITE_OIDC_CLIENT_ID_VALUE"]=$VITE_OIDC_CLIENT_ID
STRING_MAP["VITE_ALPHA_VERSION_VALUE"]=$VITE_ALPHA_VERSION
